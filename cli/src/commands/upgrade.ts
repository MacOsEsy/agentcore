...
/**
 * Command for upgrading the `agentcore` CLI to the latest version.
...
export class UpgradeCommand {
  ...
  async run(options: { dryRun?: boolean }) {
    ...
    try {
      latestVersion = execSync('npm view agentcore version', {
        encoding: 'utf8',
      }).trim();
    } catch {
  ...
  private getUpgradeCommand(pm: 'npm' | 'pnpm' | 'yarn'): string {
    switch (pm) {
      case 'pnpm':
        return 'pnpm add -g agentcore@latest';
      case 'yarn':
        return 'yarn global add agentcore@latest';
      default:
        return 'npm install -g agentcore@latest';
    }
  }
  ...
  private printManualInstructions(pm: 'npm' | 'pnpm' | 'yarn') {
    ...
    if (pm !== 'npm') {
      console.log(pc.gray('\nAlternative (npm):'));
      console.log(
        pc.gray(`  ${sudoPrefix}npm install -g agentcore@latest`),
      );
    }

    console.log(pc.gray('\nOr run via npx (no install required):'));
    console.log(pc.gray(`  npx agentcore@latest sync`));
  }
}
